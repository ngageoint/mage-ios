...
class ObservationEditCoordinatorTests: AsyncMageCoreDataTestCase {
    ...
    @MainActor
    func testInitializeTheCoordinatorWithAGeometry() {
//    it("initialize the coordinator with a geometry") {
    ...
    @MainActor
    func testInitializeTheCoordinatorWithAnObservation() {
//    it("initialize the coordinator with an observation") {
    ...
    @MainActor
    func testShouldNotAllowAUserNotInTheEventToEditAnObservation() {
//    it("should not allow a user not in the event to edit an observation") {
    ...
    @MainActor
    func testShouldAllowAUserInTheEventToEditAnObservation() {
...
//            MagicalRecord.save(blockAndWait: { (localContext: NSManagedObjectContext) in
                let observation1 = ObservationBuilder.createPointObservation(eventId: 1);
                ObservationBuilder.setObservationDate(observation: observation1, date: Date(timeIntervalSince1970: 10000000));
//            })
            try? context.obtainPermanentIDs(for: [observation1])
            try? context.save()
            let observation: Observation! = try! context.fetchFirst(Observation.self)
            return observation
        }
    ...
    @MainActor
    func testShouldShowFormChooserWithNewObservation() {
//    it("should show form chooser with new observation") {
    ...
    @MainActor
    func testShouldShowFormChooserWithNewObservationAndPickAForm() {
//    it("should show form chooser with new observation and pick a form") {
    ...
    @MainActor
    func testShouldShowFormChooserWithNewObservationAndPickAFormAndSelectAComboField() {
//    xit("should show form chooser with new observation and pick a form and select a combo field") {
    ...
    @MainActor
    func testShouldShowFormChooserWIthNewObservationAndPickAFormAndSelectTheObsrevationGeometryField() {
//    xit("should show form chooser with new observation and pick a form and select the observation geometry field") {
        ...
        tester().expect(self.viewTester().usingLabel("geometry value")!.view, toContainText: "40.1000, -105.2600 ")
    }
    
    @MainActor
    func testShouldShowFormChooserWithNewObservationAndPickAFormAndSelectAGeometryField() {
//    xit("should show form chooser with new observation and pick a form and select a geometry field") {
        ...
        tester().waitForView(withAccessibilityLabel: "Latitude");
        tester().enterText("", intoViewWithAccessibilityLabel: "Latitude", traits: [], expectedResult: "Latitude")
        tester().enterText("40.0", intoViewWithAccessibilityLabel: "Latitude", traits: [], expectedResult: "Latitude")
        tester().enterText("", intoViewWithAccessibilityLabel: "Longitude", traits: [], expectedResult: "Longitude")
        tester().enterText("-105.26", intoViewWithAccessibilityLabel: "Longitude", traits: [], expectedResult: "Longitude")
        // need to wait so that the text field can change the geometry.
        // TODO: Fix that
        tester().wait(forTimeInterval: 1.0);
        tester().waitForTappableView(withAccessibilityLabel: "Apply");
        ...
        TestHelpers.printAllAccessibilityLabelsInWindows();
        tester().expect(self.viewTester().usingLabel("field1 value")!.view, toContainText: "40.0000, -105.2600 ")
    }
...
//    xit("should show form chooser with new observation and pick a form and set the observations date") {
        ...
        formatterWithSeconds.formatOptions =  [.withInternetDateTime, .withFractionalSeconds, .withTimeZone]
    
        let date = formatter.date(from: "2020-11-02T07:00Z")!;
    ...
    @MainActor
    func testShouldShowFormChooserWithNewObservationAndCancelIt() {
...
//    it("should cancel editing") {
    ...
    @MainActor
    func testShouldNotAddArchivedFormsToTheObservation() {
