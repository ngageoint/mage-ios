...
class MageCoreDataFixtures {
    ...
    public static func addFeedToEvent(eventId: NSNumber = 1, id: NSNumber = 1, title: String = "Test Feed", completion: MRSaveCompletionHandler?) {
        MagicalRecord.save({ (localContext: NSManagedObjectContext) in
    ...
    public static func addFeedItemToFeed(feedId: NSNumber = 1, properties: [String: Any]?, completion: MRSaveCompletionHandler?) {
        MagicalRecord.save({ (localContext: NSManagedObjectContext) in
            ...
            if let f: FeedItem = FeedItem.mr_createEntity(in: localContext) {
                ...
                f.simpleFeature = SFPoint(x: 1, andY: 1);
            }
        }, completion: completion)
    }
}
