...
class ObservationLocationCompactView: UIView {
    private var constructed = false;
    private var didSetUpConstraints = false;
    private var observation: Observation?;
    ...
    private lazy var observationLocationSummaryView: ObservationLocationSummaryView = {
        let summary = ObservationLocationSummaryView();
        summary.isUserInteractionEnabled = false;
        return summary;
    }()
    ...
    func applyTheme(withScheme scheme: MDCContainerScheming?) {
        self.scheme = scheme;
        importantView.applyTheme(withScheme: scheme);
        observationLocationSummaryView.applyTheme(withScheme: scheme);
        observationActionsView.applyTheme(withScheme: scheme);
        attachmentSlideshow.applyTheme(withScheme: scheme);
    }
    
    @objc public func configure(observationLocation: ObservationLocation, scheme: MDCContainerScheming?, actionsDelegate: ObservationActionsDelegate?, attachmentSelectionDelegate: AttachmentSelectionDelegate?) {
        self.observationLocation = observationLocation
        self.observation = observationLocation.observation;
        self.actionsDelegate = actionsDelegate;
        if let observation = observationLocation.observation, observation.isImportant {
            importantView.populate(observation: observation);
            importantView.isHidden = false;
        } else {
            importantView.isHidden = true;
        }
        observationLocationSummaryView.populate(observationLocation: observationLocation);
        observationActionsView.populate(observation: observation, delegate: actionsDelegate);
        attachmentSlideshow.applyTheme(withScheme: scheme)
//        if includeAttachments, let attachments = observation.attachments, attachments.filter({ attachment in
//            attachment.url != nil
...
//        }
    ...
    func construct() {
        if (!constructed) {
            self.addSubview(stackView)
            self.stackView.addArrangedSubview(importantView);
            self.stackView.addArrangedSubview(observationLocationSummaryView);
//            self.stackView.addArrangedSubview(attachmentSlideshow);
//            self.stackView.addArrangedSubview(observationActionsView);
            setNeedsUpdateConstraints();
            constructed = true;
